...
class BuyPageEnhanced {
  ...
  setupEventListeners() {
    ...
    if (walletAddress) {
      walletAddress.addEventListener('input', e =>
        this.validateWalletAddress(e)
      );
    }
  ...
  validateAmount(amount) {
    const amountInput = document.getElementById('amount');
    const validation =
      amountInput.parentElement.querySelector('.input-validation');
  ...
  updatePurchaseSummary(amount) {
    ...
    const paymentFee =
      (amount * this.paymentMethods[this.selectedPaymentMethod].fee) / 100;
    ...
    document.getElementById('summary-tokens').textContent =
      `${tokens.toLocaleString()} GOO`;
    document.getElementById('summary-fee').textContent =
      `$${paymentFee.toFixed(2)}`;
    document.getElementById('summary-network-fee').textContent =
  ...
  calculateSavings(amount) {
    ...
    const currentFee = this.selectedPaymentMethod
      ? this.paymentMethods[this.selectedPaymentMethod].fee
      : 0;
  ...
  updatePriceDisplay() {
    ...
    document.getElementById('form-price').textContent =
      `$${this.currentPrice.toFixed(2)}`;
  ...
  setupProgressIndicator() {
    ...
    const updateProgress = () => {
      ...
      if (
        amountInput &&
        amountInput.value &&
        parseFloat(amountInput.value) >= 10
      ) {
      ...
      if (
        walletInput &&
        walletInput.value &&
        this.isValidWalletAddress(walletInput.value)
      ) {
  ...
  showSuccessModal(transactionData) {
    ...
    if (purchasedAmount)
      purchasedAmount.textContent = `${transactionData.tokens} GOO`;
    if (totalCost)
      totalCost.textContent = `$${transactionData.total.toFixed(2)}`;
