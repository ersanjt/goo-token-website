...
class WebsiteAuditor {
    ...
    for (const file of this.cssFiles) {
      if (fs.existsSync(file)) {
        ...
        this.results.css[file] = {
          ...
          hasResponsive:
            content.includes('max-width') || content.includes('min-width'),
        };
      } else {
    ...
    for (const file of this.htmlFiles) {
      if (fs.existsSync(file)) {
        ...
        this.results.consistency[file] = {
          ...
          hasProfessionalHeader: content.includes(
            'css/professional-header.css'
          ),
    ...
    for (const [file, expectedContent] of Object.entries(uniqueContent)) {
      if (fs.existsSync(file)) {
        ...
        this.results.uniqueness[file] = {
          hasUniqueContent: expectedContent.some(item =>
            content.includes(item)
          ),
          uniqueElements: expectedContent.filter(item =>
            content.includes(item)
          ),
          hasPageSpecificCSS: content.includes(
            `css/${file.replace('.html', '')}`
          ),
          hasPageSpecificJS: content.includes(
            `js/${file.replace('.html', '')}`
          ),
        };
      }
    }
  ...
  async generateReport() {
    ...
    console.log(
      `✅ ${existingHtml.length}/${htmlFiles.length} HTML files exist`
    );
    ...
    console.log(
      `✅ ${existingJs.length}/${jsFiles.length} JavaScript files exist`
    );

    // Consistency Check
    console.log('\n🔄 Consistency Analysis:');
    const consistentPages = Object.entries(this.results.consistency).filter(
      ([_, data]) =>
        data.headerElements.every(Boolean) && data.footerElements.every(Boolean)
    );
    console.log(
      `✅ ${consistentPages.length}/${Object.keys(this.results.consistency).length} pages have consistent header/footer`
    );

    // Uniqueness Check
    console.log('\n🎯 Uniqueness Analysis:');
    const uniquePages = Object.entries(this.results.uniqueness).filter(
      ([_, data]) => data.hasUniqueContent
    );
    console.log(
      `✅ ${uniquePages.length}/${Object.keys(this.results.uniqueness).length} pages have unique content`
    );
    ...
    for (const [file, data] of Object.entries(this.results.html)) {
      if (data.exists) {
        ...
        console.log(
          `  Has Feather Icons: ${data.hasFeatherIcons ? '✅' : '❌'}`
        );
        ...
        console.log(
          `  Has Language Switcher: ${data.hasLanguageSwitcher ? '✅' : '❌'}`
        );
        console.log(
          `  Has Price Display: ${data.hasPriceDisplay ? '✅' : '❌'}`
        );
    ...
    if (
      ...
      consistentPages.length === Object.keys(this.results.consistency).length
    ) {
      console.log(
