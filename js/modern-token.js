...
class ModernTokenWebsite {
  ...
  setupScrollAnimations() {
    ...
    document.querySelectorAll('.feature-card, .tokenomics-card, .roadmap-item, .market-stat, .adoption-card').forEach(el => {
  ...
  setupTokenPriceTracker() {
    const priceElements = document.querySelectorAll('.stat-value, .price-value');
    ...
    const updatePrice = () => {
      ...
      const changeElements = document.querySelectorAll('.price-change, .stat-value.positive, .stat-value.negative');
      changeElements.forEach(element => {
        if (element.textContent.includes('%') || element.textContent.includes('+') || element.textContent.includes('-')) {
          ...
          element.className = change >= 0 ? 'price-change positive' : 'price-change negative';
        }
      });
    };
  ...
  setupLivePurchaseForm() {
    ...
    const updatePurchaseInfo = () => {
      ...
      const price = 1.00; // $1.00 per token
    ...
    purchaseBtn.addEventListener('click', () => {
      ...
      if (amount > 0) {
        this.showNotification(`Purchase initiated for ${amount} GOO tokens!`, 'success');
  ...
  showPurchaseModal() {
    ...
    amountInput.addEventListener('input', () => {
      const amount = parseFloat(amountInput.value) || 0;
      const cost = amount * 1.00; // $1.00 per token
  ...
  setupModernInteractions() {
    ...
    const cards = document.querySelectorAll('.feature-card, .tokenomics-card, .market-stat, .adoption-card');
